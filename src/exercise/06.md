...
Check notes on fix "perf death by a thousand cuts", there's couple of artilces included on how state is being preserved and some more.

HOC, a higher-order component is a function that takes a component and returns a new component.

Extra credit #3 is very powerful when we want to create a generic wrapper where we get a state slice we need to pass to its child component so that identical children components are not getting rendered but just a child if the state is changed for it.

Recoil is nice library to manage some state
