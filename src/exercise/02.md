...
It's a good idea to undestand how much time a particular computation might take, for this reason, we can add console log to measure the time spent for that piece of code:

console.time('filter array');
const visibleTodos = filterTodos(todos, tab);
console.timeEnd('filter array');

Here’s the great note on how to make sure calculations are expensive and eligible for useMemo https://react.dev/reference/react/useMemo#how-to-tell-if-a-calculation-is-expensive  Should we add useMemo everywhere, great suggestions  https://react.dev/reference/react/useMemo#should-you-add-usememo-everywhere 

It's great to know about two important tools to track apps performance,
it's Profiler and Performance from Dev Tools

Hmm, it's crtainly an interesting idea to use web workers to utilize the idea of having multiple threads in JavaScript to run multiple code pieces at the same time.

useMemo and Web Workers give us flexibility to lift some weight off the main thread to make our app much more performant.
