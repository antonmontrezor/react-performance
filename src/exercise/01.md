...
Together with Suspense it's a powerful solution to optimize our app's performance especially loading times that might negativelly affect users.

const Globe = React.lazy(() => import(/* webpackPrefetch: true */ '../globe'))

The line above means that we are going to preload the component as cache when browser isn't busy right away after it served everything else needed for users to interact. Once we click on the checkbox, it'll
serve the component directly from cache.
