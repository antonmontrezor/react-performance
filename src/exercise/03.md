...
It’s possible to break down React.memo and write our own customization function to check the props for a component ListItems. This way we would just re-render an item that was previously highlighted and the newly highlighted item from the list. The problem is that requires additional effort and might be overly complicated comparing to another alternative approach. And the alternative approach is to just move the logic determining is a list item is highlighted and selected from ListItem component to Menu (parent component). That way we will be passing just isSelected and isHighlighted primitive props to every ListItem so if those change only that particular ListItem will be re-rendered which is event better than the previous approach. Also, we rely in this case on building-in React.memo comparison function and don’t need to write a custom one (check 2 screenshots below)

Overall, we should be cautious using React.memo as often it might add additional performance overhead if implemented incorrectly.
